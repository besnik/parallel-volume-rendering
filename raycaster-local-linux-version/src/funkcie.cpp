...
UINT32 dvaNaX32(UINT8 x)
{
	switch(x)
	{
		...
		//case 31: return 2147483648; break; // usigned iba v ISO C90
		// uf
										...
										float x, float y, float z, int dx, int dy, int dz)
{
	...
	{
	  return 
				 (1.0/float(dx)) * (1.0f/float(dy)) * (1.0f/float(dz)) * (
		     v000*(float(dx)-x)*(float(dy)-y)*(float(dz)-z)+
				 v100*x*(float(dy)-y)*(float(dz)-z)+
				 v010*(float(dx)-x)*y*(float(dz)-z)+
				 v001*(float(dx)-x)*(float(dy)-y)*z+
				 v011*(float(dx)-x)*y*z+
				 v101*x*(float(dy)-y)*z+
				 v110*x*y*(float(dz)-z)+
				 v111*x*y*z
...
UINT32 komCis(UINT32 n,UINT32 i)
{
	...
	if (i<n-i) i=n-i;
	int j;
	for (j=n;j>(int)i;j--) citatel*=j;
	for (j=1;j<=int(n-i);j++) menovatel*=j;
