// binswap.cpp
// by skip 2006
// samotny algoritmus binary swap sa nachadza v procesor.h/cpp
...
if (hlbka==0) {
	...
	for (int i=0; i<hlbka; i++)						// s N procesormi bude procesor komunikovat
	{
		...
		{
			if (ID-okolie>=0) // ak su obe moznosti kladne
			{
...
{
	if ((n->front==NULL) && (n->back==NULL)) printf("Data c.%d, a: %d,%d,%d, b: %d,%d,%d, i1: %d i2:%d\n",n->i1,   n->a.x, n->a.y, n->a.z,    n->b.x, n->b.y, n->b.z, n->i1, n->i2);
	else
	{
		traverzuj(n->front);
		traverzuj(n->back);
