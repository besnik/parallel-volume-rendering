...
void CZaznam::Vypis()
{
	...
	case 258:																	// hodnota = 8,16,32; pocet = pocet kanalov,
		for (UINT32 j=0;j<pocethod;j++) printf("BitsPerSample: %i\tKanal: %i\n",(int)hodnota16[j], j);
		break;
	...
	case 273:																	// hodnota = tabulka s offsetmi na pixeldata, pocet = pocet kanalov
		...
		{
			for (UINT32 j=0;j<pocethod;j++) printf("StripOffsets(16bit): %i\tKanal: %i\n",(int)hodnota16[j],j);
		} else																	// long 32bit
		{
			for (UINT32 j=0;j<pocethod;j++) printf("StripOffsets: %i\tKanal: %i\n",(int)hodnota32[j],j);
		}
		break;
	...
	case 279:																	// hodnota = pre strip pocet bytes(sirka*vysla*n kde n=1(8bit),2(12bit kanal); pocet = pocet kanalov
		if (typ==3)
		{
			for (UINT32 j=0;j<pocethod;j++) printf("StripByteCounts\tMnozstvo bytes(16bit): %i\tKanal: %i\n",(int)hodnota16[j],j);
		} else
		{
			for (UINT32 j=0;j<pocethod;j++) printf("StripByteCounts\tMnozstvo bytes: %i\tKanal: %i\n",(int)hodnota32[j],j);
		}
		break;
...
int CImageDirectory::LoadPixelData(FILE *f)								// konstruktor nenacita samotne pixel data, treba ich nacitat dodatocne touto fciou
{
	...
	for (int i=0;i<pocetkan;i++)										// vytvorenie spajaneho zoznamu
	{
    ...
    if (f!=NULL)
 	{
		...
		while (data32!=0)																// ak uz nie je dalsi image dir tak je offset rovny nule
		{
...
int _tmain(int argc, _TCHAR* argv[]) 
{
	...
	lsmfile=new CLSMFile("data.lsm",&i);
