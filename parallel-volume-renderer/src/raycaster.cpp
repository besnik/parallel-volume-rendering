...
void CRaycaster::initCamRot(int rozm_x, int rozm_y, int rozm_z)
{
  cam_old_dx = cam_old_dy = 0.0f;
	cam_dy = 0.0f;
	cam_dx = 0.0f;
	// ak su nacitane data tak prisposobime vzdialenost kamery podla nich
	if (data.data!=NULL) cam_old_vz = cam_vz = odmocnina(mocnina(rozm_x)+mocnina(rozm_y)+mocnina(rozm_z));
...
void CRaycaster::initTexture()
{
	...
	textura.sirka=scr_x;
	textura.vyska=scr_y;
	velkost = textura.sirka * textura.vyska * 4; // *3=RGBA, intenzitu= *1
...
void CRaycaster::renderuj() // kolme premietanie
{
	...
	UINT32 tex=0; // index do datoveho pola textury
...
void CRaycaster::renderujOld()
{
	CVektor3 d=CVektor3(1,1,1);
        cout << "(pocet lucov: " << scr_x*scr_y << ")" << endl;
        cout.flush();

        // konverzia z camera space start point na world start point
	float nu = this->scr_x/(2.0f*dlzka(kamera.u))*kamera.zoom*(256.0f/scr_x);
	float nv = this->scr_y/(2.0f*dlzka(kamera.v))*kamera.zoom*(256.0f/scr_y);
//        float nu = this->scr_x/(2.0f*dlzka(kamera.u));
//        float nv = this->scr_y/(2.0f*dlzka(kamera.v));

        // prechadzame vsetkymi pixelmi obrazovky
        register UINT16 ray_u, ray_v; // pixel na obrazovke [ray_u, ray_v]
        register UINT32 tex=0; // index do datoveho pola textury
	float alpha;
        cout << "kamera: "; vypis(kamera.from); cout << endl;
        ...
        for (ray_v=scr_y; ray_v>0; ray_v--)
                for (ray_u=0; ray_u<scr_x; ray_u++)
                {
                        // zistenie startovacej pozicie v cam space. scr_x, scr_y namapovane na <-1,1>
      cam_u=2.0f*(float)ray_u/(float)(scr_x-1)-1.0f;
                        cam_v=2.0f*(float)(ray_v-1)/(float)(scr_y-1)-1.0f; // -2 lebo ideme od scr_y po 1a to cele treba o -1 znizit
                        // vypocet startovacej pozicie luca v world space - vektor o ktory posunieme "from"
                        ray_start.x=nu*kamera.u.x*cam_u + nv*kamera.v.x*cam_v;
                        ray_start.y=nu*kamera.u.y*cam_u + nv*kamera.v.y*cam_v;
                        ray_start.z=nu*kamera.u.z*cam_u + nv*kamera.v.z*cam_v;
                        // nastavenie luca - zatial bez spravnej startovacej pozicie
                        r.pos=kamera.from; r.dir=kamera.dir;
                        // nastavenie spravnej startovacej pozicie
                        r.pos = r.pos + ray_start;

                        //vypis(ray_start); cout << endl;
                        //cout << endl;vypis(r.pos);
                  //vypis(r.dir); cout << endl;
                        ...
                        if (kvaderRay(data.obal,r,&tmin,&tmax))
                        {
                                // zistime vystup aby sme vedeli overi, ci sme uz von z dat
                                vystup = r.pos + r.dir*tmax;
                                // pozicia vstupu do dat
                                r.pos=r.pos + r.dir*tmin;
                                // reset poctu odobratych vzoriek
                                ...
                                // prechadzame pozdlz luca dokial sme neodobrali dostatok vzoriek a tietovzorky este prispievaju do vyslednej farby
                                while ((poc_vz<max_vz) && (acc_opacity<0.95f))
                                {
                                        ...
                                        vratOffsetBloku(r.pos,mi,mj,mk,offset);
                                        // trilinearna interpolacia intenzit
                                        farba=triInt(data.data[getIndex(mi,mj,mk,data)],data.data[getIndex(mi+1,mj,mk,data)],
                                                                                         data.data[getIndex(mi+1,mj,mk+1,data)],data.data[getIndex(mi,mj,mk+1,data)],
                                                                                         data.data[getIndex(mi,mj+1,mk,data)],data.data[getIndex(mi+1,mj+1,mk,data)],
                                                                                         data.data[getIndex(mi+1,mj+1,mk+1,data)],data.data[getIndex(mi,mj+1,mk+1,data)],
                                                                                         offset.x,offset.y,offset.z);
                                        // kompozicia
                                        //cout << "COLOR: " << farba << endl;
                                        alpha=edt_lut[(UINT16)farba];
                                        //cout << "ALPHA: " << (long)alpha << endl;
                                        acc_color=acc_color+(farba/4096.0f)*alpha*(1.0f-acc_opacity);
                                        acc_opacity=acc_opacity+alpha*(1.0f-acc_opacity);
                                        // zvysime pocet odobranych vzoriek
                                        poc_vz++;
